        ...
        if (intent.action == SaveReminderFragment.ACTION_GEOFENCE_EVENT) {
            ...
            geofencingEvent?.triggeringGeofences?.let {
                ...
                if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
                    val geofences =
                        Collections.unmodifiableList(geofencingEvent.triggeringGeofences)
...
//        Interaction to the repository has to be through a coroutine scope
        CoroutineScope(coroutineContext).launch(SupervisorJob()) {
            ...
            for (triggeringGeofence in triggeringGeofences) {
                Log.i("sendNotification", "triggred")
